...
class Auction extends Rooms.SimpleRoomGame {
  ...
  generateUsernameList(players, max = players.length, clickable = false) {
    ...
    buf += players.slice(0, max).map((p) => {
      if (typeof p === "object") {
        return `<username title="Tiers: ${p.tiers?.length ? `${import_lib.Utils.escapeHTML(p.tiers.join(", "))}` : "N/A"}"${clickable ? ' class="username"' : ""}>${import_lib.Utils.escapeHTML(p.name)}</username>`;
      }
  ...
  generatePriceList() {
    ...
    for (const player of players) {
      table += import_lib.Utils.html`<tr><td>${player.name}</td><td>${player.price}</td></tr>`;
    }
    table += `</table>`;
  ...
  nominate(user, target) {
    ...
    if (this.type === "snake") {
      ...
      this.room.add(import_lib.Utils.html`|notify|${this.room.title} Auction|${player.name} has been nominated!`);
